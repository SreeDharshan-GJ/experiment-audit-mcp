...
- **`test_connection` is exposed as a callable tool for mid-session (and first-call) re-checks.** As implemented, it is *not* invoked automatically on server start — only presence of `WANDB_API_KEY` is fail-fast-checked at startup (`auth.py`); actual credential validity against the live W&B API is only confirmed when `test_connection` is explicitly called. This entry originally described an auto-run-on-start goal that was not carried into `server.py`'s `build_server()`/`main()`; corrected here during the documentation audit (Audit #7) rather than left silently contradicting the shipped behavior. Revisit as a real Revision entry if auto-run-on-start is still wanted for v2.
