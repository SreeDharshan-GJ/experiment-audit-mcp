...
from experiment_audit.reasoning.rules import RuleContext, RuleResult, ScientificRule
from experiment_audit.reasoning.scientific_rules import (
    ConfidenceRule,
    ContradictionRule,
    JudgmentRule,
    MissingEvidenceRule,
    ...
    def build_initial_context(
        *,
        ...
        Args:
            ...
            observations: Any `Observation`s already extracted for this
                pass, exactly as `RuleContext` itself accepts them.
                Defaults to an empty `ObservationSet`, since none of
                R001-R006 read this field -- it exists on `RuleContext`
                for a rule that chooses to use it, not because this
                pipeline's own rules require it populated.
            hypotheses: Any `Hypothesis`es already generated for this
                pass, exactly as `RuleContext` itself accepts them.
                Defaults to an empty `HypothesisSet`, for the same
                reason as `observations`.
        ...
        return RuleContext(
            ...
            hypotheses=hypotheses if hypotheses is not None else HypothesisSet(),
